...
self.addEventListener('fetch', e => {
  ...
  if (url.pathname.startsWith(BASE)) {
    ...
    if (isHTML) {
      // [FIX 2026-09-10] เดิม fetch(req) เฉยๆ — "network-first" ในทางโค้ดก็จริง แต่ fetch()
      // เปล่าๆ แบบนี้ยังเคารพ HTTP cache ปกติของ browser อยู่ (คนละชั้นกับ Service Worker
      // Cache API ที่ควบคุมอยู่แล้วด้านบน) ถ้า GitHub Pages ส่ง header cache มา หรือ CDN
      // (Fastly) ของมันยังไม่ invalidate ของเก่า browser ก็ตอบจาก HTTP cache ให้เงียบๆ ได้
      // โดยไม่มีการต่อ network จริงเลย ทำให้ index.html ค้างเวอร์ชันเก่าได้ถึงจะเขียน "network-first"
      // ไว้ก็ตาม — ครอบด้วย {cache:'no-store'} เพื่อบังคับข้าม HTTP cache ทุกชั้น รับประกันว่า
      // ทุก request คือ round-trip ใหม่จริงๆ ไปที่ server เสมอ
      e.respondWith(
